...
mod tests {
    ...
    /// forces [`Transaction::skip_base_fee_check`] to return `true`. Used to exercise the
    /// per-transaction basefee bypass in [`validate_tx_env`].
    struct SkipBaseFeeTx(TxEnv);

    impl context_interface::Transaction for SkipBaseFeeTx {
        type AccessListItem<'a> = <TxEnv as context_interface::Transaction>::AccessListItem<'a>;
        type Authorization<'a> = <TxEnv as context_interface::Transaction>::Authorization<'a>;
    ...
    fn test_default_tx_rejected_below_basefee() {
        // Control: a plain TxEnv (skip_base_fee_check == false) with gas_price 0 under a
        // non-zero basefee is rejected by the basefee check.
        let mut ctx = Context::mainnet()
            .modify_block_chained(|b| b.basefee = 7)
    ...
    fn test_skip_base_fee_check_bypasses_basefee() {
        // A tx whose `skip_base_fee_check` returns true passes validation with gas_price 0
        // under a non-zero basefee, without touching `Cfg::disable_base_fee`.
        let mut ctx = Context::mainnet()
            .modify_block_chained(|b| b.basefee = 7)
